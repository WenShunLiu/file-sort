...
const vueOption = async (oldfile, newfolder) => {
  ...
  fsmkAndwrite(`${newfolder}/index.js`, `import ${indexjsName} from './index.vue' \n \nexport default ${indexjsName}`)
  ...
  cssfilesInfo.forEach(item => {
    fsmkAndwrite(`${newfolder}/${item.filename}`, item.data)
  })
  // 创建index.vue
  // 获取.vue文件中的style标签
  ...
  fsmkAndwrite(`${newfolder}/index.vue`, vueData)
}
